...
namespace ERPBase
{
    public class stoktanimclass : basetanimclass
    {
        ...
        public override void LoadData()
        {
            ...
            cmm.Connection = con;
            cmm.CommandText = "select a.id, a.kod, a.aciklama from stoktanim a ";
            da = new myDataAdapter();
            da.SelectCommand = cmm;
            da.Fill(XdtMain);
            XdtMain.Columns["kod"].Unique = true;
            da.CreateCommand();
        ...
        protected override void XdtMain_RowChanged(object sender, DataRowChangeEventArgs e)
        {
            if (e.Action == DataRowAction.Add)
            {
                if (e.Row["id"] == DBNull.Value || e.Row["id"] == null)
                    e.Row["id"] = helper.GetId("stoktanim");
